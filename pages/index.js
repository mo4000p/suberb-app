...
export default function SuperB() {
  ...
  useEffect(() => {
    if (!botActive) return;
    const msgs = [
      `⚡ Scanning ${Math.floor(Math.random() * 200 + 700)} Base tokens...`,
      '🔍 Analyzing BRETT/USDC...',
      '🛡️ Safety checking TOSHI...',
      '📊 Signal detected on DEGEN — evaluating...',
      '✅ MOCHI passed safety — calculating entry...',
    ];
    const interval = setInterval(() => {
      setStatusLine(msgs[Math.floor(Math.random() * msgs.length)]);
    }, 3000);
  ...
  const c = {
    orange: '#E85404', black: '#080810', dark: '#0D0D1A',
    card: '#111120', card2: '#16162A', border: '#1E1E35',
    text: '#F0F0FF', muted: '#6666AA', green: '#00E5A0', red: '#FF3355',
  };
    ...
    <>
      ...
      <div style={s.body}>
        ...
        <div style={s.header}>
          ...
          <div style={s.pill}>
            <div style={s.dot} className={botActive ? 'pulse-dot' : ''} />
            <span>{botActive ? 'SCANNING' : 'PAUSED'}</span>
          </div>
        </div>
        ...
        <div style={s.content}>
          ...
          {page === 'home' && <>
            <div style={s.hero}>
              ...
              <div style={{fontFamily:"'Syne',sans-serif", fontSize:48, fontWeight:800, letterSpacing:-2, lineHeight:1, marginBottom:4}}>
                $<span style={{color:c.orange}}>247.83</span>
              </div>
            ...
            <div style={s.botCard}>
              <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:12}}>
                ...
                <button style={{width:44, height:24, background: botActive ? c.orange : c.border, borderRadius:12, position:'relative', cursor:'pointer', border:'none'}}
              ...
              <div style={{fontFamily:'monospace', fontSize:11, color: botActive ? c.green : c.muted, marginBottom:10}}>
                {botActive ? statusLine : '⏸ Bot paused — no trades will execute'}
              </div>
              <div style={s.progressBar}>
                {botActive && <div className="scan-fill" style={{height:'100%', background:c.orange, borderRadius:2}} />}
              </div>
            </div>

            <div style={s.actionRow}>
              <div style={{background:c.orange, border:`1px solid ${c.orange}`, borderRadius:12, padding:14, cursor:'pointer', textAlign:'center'}} onClick={() => setModalOpen(true)}>
                <div style={{fontSize:20, marginBottom:4}}>💰</div>
                <div style={{fontSize:12, fontWeight:700, color:'white'}}>Deposit</div>
                <div style={{fontSize:10, color:'rgba(255,255,255,0.7)'}}>Add funds</div>
              </div>
              <div style={{background:c.card, border:`1px solid ${c.border}`, borderRadius:12, padding:14, cursor:'pointer', textAlign:'center'}} onClick={() => setPage('settings')}>
                <div style={{fontSize:20, marginBottom:4}}>📤</div>
                <div style={{fontSize:12, fontWeight:700}}>Withdraw</div>
                <div style={{fontSize:10, color:c.muted}}>Pull funds out</div>
              </div>
            </div>
              ...
              {trades.slice(0,3).map((t,i) => (
                <div key={i} style={s.tradeItem}>
                  <div style={{display:'flex', alignItems:'center', gap:10}}>
                    ...
                    <div>
                      <div style={{fontFamily:'monospace', fontSize:12, fontWeight:700}}>{t.token}</div>
                      <div style={{fontSize:10, color:c.muted}}>{t.time}</div>
                    </div>
                  </div>
                  <div style={{textAlign:'right'}}>
                    <div style={{fontFamily:'monospace', fontSize:13, fontWeight:700, color: t.win ? c.green : c.red}}>{t.pnl}</div>
                    <div style={{fontSize:10, color:c.muted}}>{t.pct}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={s.section}>
              <div style={{...s.card, marginBottom:16, display:'flex', alignItems:'center', gap:16}}>
                <svg width="80" height="80" viewBox="0 0 80 80">
                  ...
                  <circle cx="40" cy="40" r="32" fill="none" stroke="#00E5A0" strokeWidth="8"
              ...
              <div style={s.card}>
                ...
                <input type="range" min="1" max="100" value={Math.round(riskAmount * 10)}
                  onChange={e => setRiskAmount(e.target.value * 0.1)} />
                <div style={{display:'flex', gap:6, marginTop:10}}>
                  {[0.10, 0.50, 1.00, 5.00, 10.00].map(v => (
                    <div key={v} onClick={() => setRiskAmount(v)}
                      style={{flex:1, background: Math.abs(riskAmount - v) < 0.01 ? c.card2 : c.card2, border:`1px solid ${Math.abs(riskAmount - v) < 0.01 ? c.orange : c.border}`, borderRadius:8, padding:'6px 4px', fontFamily:'monospace', fontSize:10, color: Math.abs(riskAmount - v) < 0.01 ? c.orange : c.muted, cursor:'pointer', textAlign:'center'}}>
                      ${v.toFixed(2)}
                    </div>
                  ))}
                </div>
            ...
            <div style={s.chatArea}>
              ...
              <div style={s.chatInputRow}>
                <input style={s.chatInput} value={chatInput} onChange={e => setChatInput(e.target.value)}
                  ...
                  placeholder="Ask or train your bot..." />
            ...
            <div style={s.section}>
              ...
              <div style={{display:'flex', flexWrap:'wrap', gap:8}}>
                {["What are you watching?","My stats","Be more aggressive","Avoid memecoins","Pause trading","Resume trading"].map(cmd => (
                  <div key={cmd} onClick={() => { setChatInput(cmd); setTimeout(() => sendMessage(), 100); }}
                    style={{background:c.card2, border:`1px solid ${c.border}`, borderRadius:8, padding:'8px 12px', fontFamily:'monospace', fontSize:10, color:c.muted, cursor:'pointer'}}>
                    {cmd}
                  </div>
                ))}
              </div>
          ...
          {page === 'settings' && <div style={{padding:20}}>
            ...
            <div style={{...s.card, marginBottom:12}}>
              ...
              <div style={{fontFamily:'monospace', fontSize:12}}>0xA614bd...Aaf70</div>
              <div style={{fontSize:10, color:c.muted, marginTop:4}}>Base Network · superb.base.eth</div>
            </div>
            <div style={{...s.card, marginBottom:12}}>
              <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12}}>
                <div style={{fontSize:13, fontWeight:700}}>Vault Balance</div>
                <div style={{fontFamily:'monospace', fontSize:16, color:c.orange}}>$247.83</div>
              </div>
              <button onClick={() => setModalOpen(true)} style={{width:'100%', background:c.orange, border:'none', borderRadius:12, padding:14, fontFamily:"'Syne',sans-serif", fontSize:15, fontWeight:700, color:'white', cursor:'pointer', marginBottom:8}}>Deposit Funds</button>
              <button style={{width:'100%', background:'transparent', border:`1px solid ${c.border}`, borderRadius:12, padding:12, fontFamily:"'Syne',sans-serif", fontSize:13, color:c.muted, cursor:'pointer'}}>Withdraw All Funds</button>
            </div>
        ...
        {modalOpen && (
          <div style={s.modal} onClick={e => e.target === e.currentTarget && setModalOpen(false)}>
            <div style={s.modalBox}>
              ...
              <div style={{fontSize:11, color:c.muted, marginBottom:12}}>Minimum $1.00 · Funds stay in your vault</div>
              <input type="number" placeholder="$0.00" value={depositAmount}
                onChange={e => setDepositAmount(e.target.value)}
                style={{width:'100%', background:c.card, border:`1px solid ${c.border}`, borderRadius:10, padding:14, fontFamily:'monospace', fontSize:20, color:c.text, outline:'none', marginBottom:12, textAlign:'center'}}/>
              <button onClick={() => { alert(`Connecting Base wallet to deposit $${depositAmount}...`); setModalOpen(false); }}
                style={{width:'100%', background:c.orange, border:'none', borderRadius:12, padding:16, fontFamily:"'Syne',sans-serif", fontSize:16, fontWeight:700, color:'white', cursor:'pointer', marginBottom:10}}>
                Deposit via Base Wallet
              </button>
              <button onClick={() => setModalOpen(false)}
                style={{width:'100%', background:'transparent', border:`1px solid ${c.border}`, borderRadius:12, padding:14, fontFamily:"'Syne',sans-serif", fontSize:14, color:c.muted, cursor:'pointer'}}>
                Cancel
              </button>
            </div>
          </div>
